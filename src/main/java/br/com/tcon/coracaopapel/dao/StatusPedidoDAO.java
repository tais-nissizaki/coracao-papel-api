...
@Repository
public class StatusPedidoDAO implements IDAO {
	...
	public List<EntidadeDominio> consultar(EntidadeDominio entidade) {
		if(entidade != null && entidade.getId() != null && entidade.getId() > 0) {
			StatusPedido statusPedido = entityManager.find(StatusPedido.class, entidade.getId());
	...
	private String montarQuery(EntidadeDominio entidade) {
		StringBuilder queryBase = new StringBuilder("SELECT t FROM StatusPedido t ");
		if (entidade != null) {
