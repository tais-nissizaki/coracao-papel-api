...
@Repository
public class TipoClienteDAO implements IDAO {
	...
	public List<EntidadeDominio> consultar(EntidadeDominio entidade) {
		...
		List<TipoCliente> listaResultadoQuery = (List<TipoCliente>) query.getResultList();
		List<EntidadeDominio> listaRetorno = new ArrayList<>();
		for(int i=0; i< listaResultadoQuery.size(); i++) {
			TipoCliente tipoEndereco = (TipoCliente) listaResultadoQuery.get(i);
	...
	private String montarQuery(EntidadeDominio entidade) {
		StringBuilder queryBase = new StringBuilder("SELECT t FROM TipoCliente t ");
		if (entidade != null) {
