...
public class TipoEnderecoDAO implements IDAO {
	...
	public List<EntidadeDominio> consultar(EntidadeDominio entidade) {
		...
		List<TipoEndereco> listaResultadoQuery = (List<TipoEndereco>) query.getResultList();
		List<EntidadeDominio> listaRetorno = new ArrayList<>();
		for(int i=0; i< listaResultadoQuery.size(); i++) {
			TipoEndereco tipoEndereco = (TipoEndereco) listaResultadoQuery.get(i);
			entityManager.detach(tipoEndereco);
//			tipoEndereco.setEndereco(null);
	...
	private String montarQuery(EntidadeDominio entidade) {
		StringBuilder queryBase = new StringBuilder("SELECT t FROM TipoEndereco t ");
		if (entidade != null) {
